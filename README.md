...
- `artifacts/data/top50_public_substrate_check_contract_summary.json`: aggregate non-mutating check contract that runs the current public clone, contributor worklist, Vanderbilt handoff, synthetic dry-run, slice-2 approval-request, slice-2 route-observation, slice-2 parser/scope approval request, slice-2 approved next-packet ledger, and fail-closed gates as one CI-style surface.
- `artifacts/data/top50_public_contributor_worklist_summary.json`: GBrain-approved non-mutating contributor worklist over the verified top-50/Vanderbilt substrate, ranking public-clone verification, Vanderbilt manual review packets, active gap-manifest planning, slice-2 live-fetch approval, approved slice-2 parser/scope next-packet lanes, and future exact-approval blockers.
