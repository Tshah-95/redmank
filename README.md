...
- `artifacts/data/top50_public_substrate_check_contract_summary.json`: aggregate non-mutating check contract that runs the current public clone, contributor worklist, Vanderbilt handoff, synthetic dry-run, and fail-closed gates as one CI-style surface.
- `artifacts/data/top50_public_contributor_worklist_summary.json`: GBrain-approved non-mutating contributor worklist over the verified top-50/Vanderbilt substrate, ranking public-clone verification, Vanderbilt manual review packets, active gap-manifest planning, and future exact-approval blockers.
