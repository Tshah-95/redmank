...
- `artifacts/data/top50_public_substrate_check_contract_summary.json`: aggregate non-mutating check contract that runs the current public clone, contributor worklist, Vanderbilt handoff, synthetic dry-run, slice-2 approval-request, slice-2 route-observation, slice-2 parser/scope approval request, slice-2 approved next-packet ledger, slice-2 follow-up packet set, and fail-closed gates as one CI-style surface.
- `artifacts/data/top50_public_contributor_worklist_summary.json`: GBrain-approved non-mutating contributor worklist over the verified top-50/Vanderbilt substrate, ranking public-clone verification, Vanderbilt manual review packets, active gap-manifest planning, slice-2 live-fetch approval, approved slice-2 parser/scope next-packet lanes, the combined slice-2 follow-up packet set, and future exact-approval blockers.
- `artifacts/data/top50_public_contributor_worklist_verification_summary.json`: GBrain-approved non-mutating verification summary for the contributor worklist rowset, commands, approval/prohibition gates, artifact pointers, and public-safety leak scan.
...
- `artifacts/data/school_gap_resolution_batch_slice_index_summary.json`: public-safe slice index over the 21 committed Vanderbilt gap-resolution batches, writing one bounded gap packet slice to `/tmp` by default.
- `artifacts/data/vanderbilt_open_gap_manifest_triage_packet_summary.json`: public-safe Vanderbilt triage packet ranking all 21 committed gap-resolution slices over 113 open gaps into bounded contributor lanes beyond the General Surgery reviewer handoff.
- `artifacts/data/vanderbilt_triage_slice_definition_contract_summary.json`: non-mutating slice-family contract for Vanderbilt open-gap triage, defining permitted evidence actions, `/tmp` output policy, and mutation prohibitions for source-discovery, route/parser review, scope review, and URL-confirmation slices.
- `artifacts/data/vanderbilt_slice_2_execution_plan_packet_summary.json`: no-fetch execution-plan packet for Vanderbilt triage order 2 / execution order 1, converting nine mixed-negative-evidence gaps into query, route-review, and packet-preparation instructions without fetching pages or accepting people.
- `artifacts/data/vanderbilt_slice_2_live_fetch_approval_request_packet_summary.json`: exact pending GBrain approval-request packet for future bounded live route probing over the nine slice-2 rows; it requests future route metadata/hash collection while keeping `web_fetch_allowed=false` for the packet itself and prohibiting raw dumps, parser acceptance, person ingestion, denominator closure, school verification, URL rewrites, enrichment acceptance, and identity collapse.
- `artifacts/data/vanderbilt_slice_2_live_route_observation_summary.json`: GBrain-approved non-mutating live route-observation ledger for the nine slice-2 rows, recording 18 denominator/context route observations across 12 unique official URLs with hashes, final URLs, and coarse current-roster signal counts only.
- `artifacts/data/vanderbilt_slice_2_route_parser_scope_approval_packet_summary.json`: non-mutating exact approval-request packet that classifies the 18 slice-2 route observations into target-route parser-build review, related-scope disposition, broader-context recourse, and denominator-redirect recourse lanes without authorizing parser implementation or acceptance.
- `artifacts/data/vanderbilt_slice_2_approved_parser_scope_next_packet_summary.json`: approved non-mutating ledger for the 18 slice-2 route parser/scope rows, preserving the exact GBrain approval boundary while splitting follow-up work into target-route parser-build review, related-scope disposition, broader-context recourse, and denominator-redirect recourse lanes.
- `artifacts/data/vanderbilt_slice_2_followup_packet_set_summary.json`: combined non-mutating follow-up packet set approved by GBrain option A, with row-level packet families and required evidence boundaries for all four slice-2 lanes while continuing to prohibit web fetch, parser execution, parser acceptance, person ingestion, denominator closure, school verification, URL rewrites, and identity collapse.
- `artifacts/data/school_gap_resolution_review_template_summary.json`: blank public-safe review/source-discovery template over all 113 committed Vanderbilt gap packets, with contributor fields for candidate-evidence planning only.
- `artifacts/data/school_gap_resolution_review_template_validation_summary.json`: non-mutating validator summary for the blank gap review template; filled rows must keep candidate evidence separate from person ingestion, denominator closure, school verification, URL rewrites, identity collapse, and private artifact paths.
- `artifacts/data/school_gap_resolution_targeted_review_packet_summary.json`: non-mutating targeted Vanderbilt review packet derived from the `dd72909d...` source-discovery workbench, with 19 filled candidate-evidence rows and 94 still-pending template rows.
- `artifacts/data/school_gap_resolution_targeted_review_packet_validation_summary.json`: validator summary for the targeted packet, confirming 19 valid non-mutating rows, 94 pending rows, and no invalid rows.
- `artifacts/data/school_gap_resolution_parser_scope_bridge_summary.json`: non-mutating bridge from the targeted Vanderbilt review packet into existing Vanderbilt parser/scope packets, proving 19 targeted rows are represented by 20 parser/scope review, route, and decision rows without approving parser acceptance.
- `artifacts/data/school_gap_resolution_candidate_output_bridge_summary.json`: non-mutating bridge from Vanderbilt parser/scope coverage into approved candidate-only parser diagnostics, proving 19 programs map to 20 implementation-approval rows and 159 verified candidate/scope/recourse output rows without accepting people.
- `artifacts/data/school_gap_resolution_review_queue_bridge_summary.json`: non-mutating bridge from candidate-only parser diagnostics into approved review queues, decision scaffolds, scaffold verification, and the pending manual-decision audit.
- `artifacts/data/vanderbilt_reviewer_execution_readiness_bridge_summary.json`: non-mutating bridge from pending Vanderbilt review queues into `/tmp`-only reviewer slice, extraction, dry-run, and explicit apply commands without running commands or filling decisions.
- `artifacts/data/vanderbilt_reviewer_blank_execution_verification_summary.json`: non-mutating verifier proving all 20 blank Vanderbilt reviewer slices fail closed, produce zero dry-run patch rows, and remove `/tmp` outputs.
- `artifacts/data/vanderbilt_reviewer_slice_prioritization_plan_summary.json`: non-mutating reviewer slice assignment plan ranking the 20 fail-closed Vanderbilt slices with public-safe lane, row-count, and verifier-status metrics; General Surgery is the first bounded 2-row review slice.
...
- `scripts/slice_school_gap_resolution_batch_packets.py`: selector CLI for writing exactly one school gap-resolution batch packet slice by execution order or batch key.
- `scripts/materialize_vanderbilt_open_gap_manifest_triage_packet.py`: reproducible materializer for the public-safe Vanderbilt open-gap triage packet and slice-definition contract.
- `scripts/materialize_vanderbilt_slice_2_execution_plan_packet.py`: reproducible materializer for the GBrain-selected no-fetch Vanderbilt slice-2 execution-plan packet.
- `scripts/materialize_vanderbilt_slice_2_live_fetch_approval_request_packet.py`: reproducible materializer for the pending exact GBrain approval request to run a later bounded Vanderbilt slice-2 live route probe.
- `scripts/fetch_vanderbilt_slice_2_live_route_observations.py`: approved public-route observer for Vanderbilt slice-2 denominator/context URLs; it commits sanitized metadata, hashes, and coarse signals only.
- `scripts/materialize_vanderbilt_slice_2_route_parser_scope_approval_packet.py`: reproducible materializer for the pending exact GBrain approval request over slice-2 route parser/scope and recourse lanes.
- `scripts/materialize_vanderbilt_slice_2_approved_parser_scope_next_packets.py`: reproducible materializer for the approved non-mutating Vanderbilt slice-2 parser/scope next-packet ledger.
- `scripts/materialize_vanderbilt_slice_2_followup_packet_set.py`: reproducible materializer for the combined no-fetch/no-parser-execution Vanderbilt slice-2 follow-up packet set.
- `scripts/materialize_school_gap_resolution_review_template.py`: reproducible materializer for the blank Vanderbilt gap review/source-discovery template.
- `scripts/materialize_school_gap_resolution_targeted_review_packet.py`: reproducible materializer that fills the review template from the targeted Vanderbilt source-discovery workbench without accepting people or closing gaps.
- `scripts/materialize_school_gap_resolution_parser_scope_bridge.py`: reproducible bridge materializer that reconciles the targeted review packet against existing Vanderbilt parser/scope rowsets and records remaining approval gates.
- `scripts/materialize_school_gap_resolution_candidate_output_bridge.py`: reproducible bridge materializer that reconciles parser/scope bridge rows against approved candidate-only parser outputs and verification rowsets.
- `scripts/materialize_school_gap_resolution_review_queue_bridge.py`: reproducible bridge materializer that reconciles candidate-output bridge rows against review queue, decision scaffold, verification, and pending audit rowsets.
- `scripts/materialize_vanderbilt_reviewer_execution_readiness_bridge.py`: reproducible bridge materializer that reconciles review-queue bridge rows against the reviewer workbook slice index and helper command surfaces.
- `scripts/materialize_vanderbilt_reviewer_blank_execution_verification.py`: reproducible verifier that runs the blank reviewer slice/extract/dry-run path to `/tmp` and proves it fails closed without applying decisions.
- `scripts/materialize_vanderbilt_reviewer_slice_prioritization_plan.py`: reproducible materializer for the public-safe Vanderbilt reviewer slice prioritization plan; it ranks fail-closed slices without filling decisions, applying patches, accepting people, or closing denominators.
...
- `artifacts/data/vanderbilt_reviewer_decision_patch_template_summary.json`: helper-compatible blank patch template for public-safe Vanderbilt reviewer decisions; all action and confirmation fields are intentionally empty until a reviewer fills a bounded patch and validates it through the patch helper.
- `artifacts/data/vanderbilt_reviewer_decision_patch_workbook_summary.json`: public-safe Vanderbilt reviewer patch workbook that joins operator packet context, current decision fingerprints, allowed actions, required confirmations, and blank patch fields; extract filled rows before passing them to the strict patch helper.
- `artifacts/data/vanderbilt_reviewer_decision_patch_workbook_slice_index_summary.json`: public-safe index of 20 bounded Vanderbilt workbook slice commands, each writing one operator-packet workbook to `/tmp` by default.
- `artifacts/data/vanderbilt_reviewer_slice_prioritization_plan_summary.json`: public-safe Vanderbilt reviewer slice priority plan over the 20 fail-closed workbook slices.
...
- `scripts/materialize_top50_public_substrate_check_contract.py`: aggregate CI-style check contract for the current public-safe top-50/Vanderbilt substrate gates.
...
- `scripts/materialize_top50_public_contributor_worklist_verification.py`: pure verifier for the public contributor worklist; it checks rowsets, commands, mutation gates, artifact paths, and leak-safety without executing contributor work.
- `scripts/assert_gap_manifest_fails_closed.py`: helper assertion used by the public contributor worklist to prove `materialize_school_gap_resolution_manifest.py` refuses empty output in an incomplete public checkout.
...
- `scripts/materialize_vanderbilt_reviewer_decision_patch_template.py`: reproducible materializer for the helper-compatible blank Vanderbilt reviewer decision patch template; it emits no names, URLs, notes, accepted people, or denominator mutations.
- `scripts/materialize_vanderbilt_reviewer_decision_patch_workbook.py`: reproducible materializer for the public-safe Vanderbilt reviewer patch workbook and current decision-fingerprint context.
- `scripts/materialize_vanderbilt_reviewer_decision_patch_workbook_slice_index.py`: reproducible materializer for the public-safe Vanderbilt workbook slice index.
- `scripts/slice_vanderbilt_reviewer_decision_patch_workbook.py`: selector CLI for writing exactly one operator-packet workbook slice by execution order or operator packet key.
- `scripts/materialize_vanderbilt_reviewer_slice_prioritization_plan.py`: reproducible planner that ranks verified Vanderbilt reviewer slices for bounded human review without mutating decisions.
