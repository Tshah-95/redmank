...
Run `scripts/materialize_official_profile_discovery_workbench.py` after profile search/probe collection. It turns the raw discovery artifacts into `official_profile_discovery_workbench.csv`, one row per uncovered profile-search task, with explicit lanes for planned searches, blocked search endpoints, low-signal direct probes, official-profile candidates, and no-candidate-after-search cases. `scripts/materialize_official_profile_discovery_batches.py` then groups that workbench into bounded execution batches so planned searches, endpoint retries, direct-probe reviews, and candidate reviews can be worked without losing person-level provenance. `scripts/materialize_official_profile_reobservation_audit.py` is the live-source recheck for review-ready profile URL candidates: it records the current page hash, title, canonical URL, same-name/program context checks, and route-drift status before any reviewer acceptance. `scripts/materialize_official_profile_reviewer_decision_dossiers.py` turns the current queue/audit/reobservation state into copy-ready reviewer dossiers with manual decision templates keyed to the current profile fingerprint. The corpus action worklist consumes the batch artifact when available and routes review-ready profile candidates to the dossier artifact while suppressing the older broad `official_profile_search` enrichment group when person-level rows exist.
...
`scripts/materialize_official_profile_discovery_workbench.py` is the review layer above those raw artifacts. It keeps profile candidates non-mutating, ranks official-profile review candidates above blocked or planned search rows, and preserves query/candidate evidence so the next pass can retry only stale or unresolved discovery lanes. `scripts/materialize_official_profile_discovery_batches.py` is the execution layer above the workbench; it keeps the operator surface bounded while preserving query status, candidate status, top people, and required evidence per batch. `scripts/materialize_official_profile_reobservation_audit.py` then separates current profile pages from stale route candidates; `scripts/materialize_official_profile_reviewer_decision_dossiers.py` exposes that status, the acceptance boundary, and a current-fingerprint decision template per reviewer row. The current committed pass shows the review-ready profile URLs resolving to generic `Academic Departments` pages, so those rows should be reprobed or replaced before reviewer acceptance.
