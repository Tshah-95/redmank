...
`scripts/materialize_official_profile_discovery_workbench.py` is the review layer above those raw artifacts. It keeps profile candidates non-mutating, ranks official-profile review candidates above blocked or planned search rows, and preserves query/candidate evidence so the next pass can retry only stale or unresolved discovery lanes. `scripts/materialize_official_profile_reobservation_audit.py` then separates current profile pages from stale route candidates; `scripts/materialize_official_profile_reviewer_decision_dossiers.py` exposes that status, the acceptance boundary, and a current-fingerprint decision template per reviewer row. The current committed pass shows the review-ready profile URLs resolving to generic `Academic Departments` pages, so those rows should be reprobed or replaced before reviewer acceptance.
