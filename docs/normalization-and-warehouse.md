...
- `official_program_coverage_batch_packets`: per-action packet support for coverage batches, preserving queue row, program dossier, source/alias/count evidence, target routing, and denominator acceptance boundaries.
...
- `program_lifecycle_duration_review_batch_packets`: row-level lifecycle-duration packets under review batches, preserving each queue row, source evidence, manual decision template, support status, and non-mutating acceptance boundary.
...
- `training_temporal_contract_batch_packets`: row-level packets under temporal-contract batches, preserving every batched contract key, person/program/source context, diff policy, required evidence, and support status while keeping the operator worklist batch-shaped.
...
- `official_roster_refresh_batch_packets`: per-source/program support packets under roster-refresh execution batches, preserving refresh keys, source URLs, parser support, expected state-machine burden, and target transition ledgers.
...
- `contact_verification_batch_packets`: per-contact packets under contact verification batches, preserving dossier keys, reviewer-decision keys, contact contracts, source/reobservation evidence, support status, and confirmation boundaries while keeping the operator worklist batch-shaped.
...
- `person_enrichment_execution_batch_packets`: per-readiness support packets for those execution batches, preserving person/task keys, collector requirements, target ledgers, and packet-level next actions while leaving full readiness evidence in the source readiness table.
...
`scripts/materialize_program_lifecycle_duration_reviewer_decisions.py` turns those duration candidates into the same explicit decision pattern used for alias and person-evidence review: queue rows carry evidence fingerprints and required confirmation fields, manual decisions are audited against the current fingerprint, and accepted duration mappings remain non-mutating until a later lifecycle-rule edit cites them. `scripts/materialize_program_lifecycle_duration_review_batches.py` then groups unresolved decision rows into bounded reviewer/source-evidence sessions, and `scripts/materialize_program_lifecycle_duration_review_batch_packets.py` expands each batch back to row-level packets so the worklist can route compact context-review and no-explicit-duration batches while retaining exact source evidence for every reviewer decision. This prevents a strong official-page phrase from silently rewriting the state machine while still making it easy to reduce `source_refresh_required` and `manual_review_required` burden over time.
...
- `scripts/materialize_person_evidence_review_batch_packets.py` expands those sessions into packet-level support rows so the worklist can stay batch-level while sourcing packet evidence directly, giving reviewers current packet fingerprints, top records, support status, reviewer-decision keys, and recommended actions for each member packet.
...
`scripts/materialize_person_enrichment_execution_batches.py` is the operator entry point for that readiness ledger. `scripts/materialize_person_enrichment_execution_batch_packets.py` adds per-readiness packet support under those resumable batches, so the operator worklist can stay batch-bounded while still exposing exact person/task targets, required collection/review mode, and downstream ledger route. `scripts/materialize_corpus_action_worklist.py` consumes packet support when present, so queue-level enrichment execution is routed through the same command hints, evidence requirements, recency policy, provenance policy, and downstream artifact contract that the batch materializer records.
...
`scripts/materialize_training_temporal_contract_batches.py` is the operator layer above that row ledger. It batches only `source_refresh_required` and `manual_review_required` contracts into bounded non-mutating sessions by policy lane, role, category, program, lifecycle code, and next-refresh contract. `scripts/materialize_training_temporal_contract_batch_packets.py` expands those bounded sessions back to one packet per contract row, so `scripts/materialize_corpus_action_worklist.py` can retain one row per batch while preserving exact row-level contract keys, review triggers, support status, and required evidence as downstream detail.
