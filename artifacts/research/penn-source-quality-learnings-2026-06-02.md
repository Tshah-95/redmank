...
Generated: 2026-06-02T19:11:41.510363+00:00
...
| Warehouse reproducibility provenance audit | artifact existence, row-count parity, content hashes, and repository-size pressure | 101 | 101 | 88.0 | high_utility | retain_sqlite_as_generated_untracked_artifact_and_refresh_manifest |
...
Worklist rows: 1395. Summed impact count: 12083. Critical rows: 23. High rows: 869.
...
| person_enrichment_execution | review_queue:evidence_reconciliation_review | resident evidence_reconciliation_review | resident | 1265 | 215 | review_reconciliation_packet |
| person_enrichment_execution | review_queue:evidence_reconciliation_review | resident evidence_reconciliation_review | resident | 1257 | 207 | review_reconciliation_packet |
...
| person_enrichment_execution | review_queue:evidence_reconciliation_review | fellow evidence_reconciliation_review | fellow | 1134 | 119 | review_reconciliation_packet |
...
## Roster Refresh Drift

Latest Medicine roster refresh status: 23 sources parsed successfully; 5 sources preserved previous public rows after current refresh errors. Latest broad affiliated roster refresh status: 15 sources parsed successfully; 2 sources preserved previous public rows after redirected pages no longer exposed parser-supported person structure.

Learning: official roster pages are strong current-membership anchors only when freshly observed and parsed. When a later refresh redirects to a broken URL, returns a transport error, or drops supported roster markup, the safer public-corpus behavior is to preserve the last successful public observation with explicit `latest_refresh_*` provenance and let the temporal state machine decide whether the row needs source refresh, expected advancement/completion, or review. Absence caused by fetch or parser drift should not silently become graduation, deletion, or current-status mutation.

## Prior-Training Broad Search

Latest prior-training background pass executed 40 of 642 queued searches. It recorded 38 non-200 search endpoint responses, 2 HTTP 200 search responses, and 0 candidate background pages.

Learning: broad HTML search is currently a weak first-line utility for high-fidelity medical-school or prior-residency enrichment at Penn scale. The corpus should keep the query manifest and endpoint-failure evidence, but stronger next lanes are official roster-linked profiles, public CV/biosketch pages, ORCID/profile identity bridges, and source-specific alumni/graduate directories. Search absence from this pass is not evidence that background data is unavailable.
